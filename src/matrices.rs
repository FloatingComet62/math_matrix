...
impl Matrix {
    ...
    /// let invalid_det_matrix = Matrix::new(vec![1, 2, 3], (3, 1)).unwrap();
    ///
    /// // notice the matrix can be arranged like a square
    /// let invalid2_det_matrix = Matrix::new(vec![1, 2, 3, 4], (1, 4)).unwrap();
    ///
...
impl Mul<i32> for Matrix {
    ...
    fn mul(self, rhs: i32) -> Self::Output {
        Matrix::generate(|i, j| self.get(i, j).unwrap() * rhs, self.order)
    }
}
